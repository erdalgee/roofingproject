...
// Regenerate supplier API key (admin only)
app.post('/v1/admin/suppliers/:id/regenerate-key', requireAdmin, (req, res) => {
  ...
  res.json({ 
    ...
    api_key: suppliers[index].api_key 
  });
});
...
// Supplier upload products (supplier key or admin)
app.post('/v1/supplier/upload-products', adminLimiter, requireSupplier, express.text({ type: 'text/csv', limit: '10mb' }), (req, res) => {
  if (!req.body) return res.status(400).json({ error: 'CSV data required' });
  
  const supplierName = req.supplier ? req.supplier.name : req.body.supplier_name || 'Admin';
  ...
  const results = { success: 0, created: 0, updated: 0, errors: [] };
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (existingIdx >= 0) {
      // Only update if admin or owns the product
...
app.get('/v1/supplier/my-products', requireSupplier, (req, res) => {
  const products = loadProducts();
  const myProducts = req.isAdmin 
    ? products 
    : products.filter(p => p.supplier_id === req.supplier.id);
...
app.get('/', (req, res) => {
  res.json({
    ...
    version: '2.0.0',
    features: ['Supplier-specific keys', 'Product upload', 'Vendor API'],
    endpoints: {
      admin: ['/v1/admin/suppliers', '/v1/admin/suppliers/:id/regenerate-key'],
